    ...
    <>
      ...
      <article className="flex max-w-xl flex-col items-start justify-between sm:mb-16 mt-8">
        <div className="flex items-center text-xs flex-wrap">
          <time dateTime={article.date} className="text-light_gray mr-2 sm:mr-3">
            {article.date}
          </time>
          <div className="h-4 w-0.5 bg-gray-300" />
          <p className="text-light_gray mr-2 sm:mr-3">{article.publisher}</p>
          {article.categories.map(
            (category) =>
              category && (
                <div
                  key={category.title}
                  className="relative z-10 rounded-full bg-blue-50 px-2 mr-2 sm:mr-3 py-1.5 font-medium text-primary"
                >
                  {category.title}
                </div>
              )
